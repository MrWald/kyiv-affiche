...
export var EStatsEvent;
(function (EStatsEvent) {
    EStatsEvent["Get"] = "get";
    EStatsEvent["Help"] = "help";
    EStatsEvent["Start"] = "start";
    EStatsEvent["Subscribe"] = "subscribe";
    EStatsEvent["Unsubscribe"] = "unsubscribe";
})(EStatsEvent || (EStatsEvent = {}));
...
export const logEvent = (eventName) => new Promise((resolve, reject) => {
    const now = new Date();
    const key = eventKeyWithDate(eventName, now);
    redisClient.incr(key, (err, reply) => (err ? reject(err) : resolve(reply)));
});
...
const getIntVal = (key) => new Promise((resolve, reject) => {
    redisClient.get(key, (err, val) => {
        ...
        if (!val) {
            ...
            return resolve(parseInt(val, 10));
        }
    });
});
