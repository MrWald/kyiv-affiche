...
export const moviesListFromCinemasData = (cinemas) => {
    const res = [];
...
const moviePriorityFromCinemas = (title, cinemas) => {
    let sessionsCount = 0;
    for (const cinema of cinemas) {
...
export const cinemsDataToMsg = (cinemas) => {
    ...
    return reduce(sortMoviesMsg, (memo, { msg }) => (memo ? `${memo}${RN2}${msg}` : msg), '');
};
const getMovieMsg = (title, cinemas) => {
    if (!title) {
        return null;
    }
    let str = `🍿 *${title}*`;
...
const cinemaToMovieMsg = (title, cinema) => {
...
const sessionsToFormatStr = (sessions) => {
    if (!sessions.length) {
        return null;
    }
...
const dateStrToTime = (val) => (moment(val).format('HH:mm'));
